...
class NextcloudNotificationAlerter(Alerter):
    ...
    def send_nextcloud_notification(self,shortMessage: str, longMessage: str) -> None:
        ...
        requests.post(
            f"https://{self.nextcloud_user}:{self.nextcloud_token}@{self.nextcloud_server}/{uri}/{self.nextcloud_receiver}",
    ...
    def send_alert(self, name: str, monitor: Monitor) -> None:
        ...
        if alert_type == AlertType.NONE:
            return
        shortMessage = self.build_message(AlertLength.NOTIFICATION, alert_type, monitor)
        longMessage = self.build_message(AlertLength.FULL, alert_type, monitor)

        if not self._dry_run:
            try:
                self.send_nextcloud_notification(shortMessage, longMessage)
            except Exception:
