...
class MonitorTimeFilter(Enum):
    """How should the Monitor times be handled"""
...
class Monitor:
    ...
    def __init__(
        ...
        self._times_type = MonitorTimeFilter.ALWAYS
        if _times_type == "always":
            self._times_type = MonitorTimeFilter.ALWAYS
        elif _times_type == "only":
            self._times_type = MonitorTimeFilter.ONLY
        elif _times_type == "not":
            self._times_type = MonitorTimeFilter.NOT
        else:
        ...
        if self._times_type in [
            MonitorTimeFilter.ONLY,
            MonitorTimeFilter.NOT,
        ]:
            time_lower = str(
    ...
    def should_run(self) -> bool:
        ...
        if not self.enabled:
            return False
        now = int(time.time())
        if self.time_shouldrun():
            self.maintenance = False
        else:
            self.maintenance = True
            self._state = MonitorState.MAINTENANCE
            maint_time = self.describe_times()
            self.last_result = f"In Maintenance Mode, checked {maint_time}"
            return False
        if self._force_run:
    ...
    def describe_times(self) -> str:
        ...
        if self._times_type == MonitorTimeFilter.ALWAYS:
            return "(always)"
        ...
        if self._times_type == MonitorTimeFilter.ONLY:
    ...
    def allowed_time(self) -> bool:
        ...
        if self._time_info[0] is not None and self._time_info[1] is not None:
            ...
            if self._times_type == MonitorTimeFilter.ONLY:
                self.monitor_logger.debug("in_time_range: %s", in_time_range)
                return in_time_range
            if self._times_type == MonitorTimeFilter.NOT:
    ...
    def allowed_day(self) -> bool:
        """Check if today is an allowed day."""
        if arrow.now(self._times_tz).weekday() not in self._days:
            self.monitor_logger.debug("not allowed to alert today")
            return False
        return True

    def time_shouldrun(self) -> bool:
        """Call allowed day/time functions and return a true if we should run
        False if in Maintenance"""
        if self.allowed_day() and self.allowed_time():
            return True
        return False
